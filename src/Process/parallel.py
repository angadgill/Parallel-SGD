...
3)Find the solution for floating point problem
4)Code review and documentation
"""

def _average_sgd_wrapper(coef, intercept, average_coef, average_intercept,
                         loss_function,
...
def parallel_average_sgd(coef, intercept, average_coef, average_intercept, loss_function,
    ...
    for i in range(0, 10):
        p.insert(0, Process(target=_average_sgd_wrapper, args=(coef, intercept, average_coef, average_intercept,
                                                               loss_function,
                                                               penalty_type, alpha, C, l1_ratio,
                                                               dataset, n_iter, int(fit_intercept),
                                                               int(verbose), int(shuffle), seed,
                                                               pos_weight, neg_weight,
                                                               learning_rate_type, eta0,
                                                               power_t, t_, intercept_decay, average, coefArray,
                                                               interceptArray, i)))
        p[0].start()
...
def _plain_sgd_wrapper(coef, intercept, loss_function,
                       penalty_type, alpha, C, l1_ratio,
                       dataset, n_iter, fit_intercept,
                       verbose, shuffle, seed,
                       pos_weight, neg_weight,
...
def fit_binary(est, i, X, y, alpha, C, learning_rate, n_iter,
    ...
    if not est.average:
        return parallel_plain_sgd(coef, intercept, est.loss_function,
        ...
        average_intercept = parallel_average_sgd(coef, intercept, average_coef,
                                                 average_intercept,
    ...
    def _fit_regressor(self, X, y, alpha, C, loss, learning_rate,
        ...
        if self.average > 0:
            ...
            self.average_coef_, self.average_intercept_ = \
                parallel_average_sgd(self.standard_coef_,
